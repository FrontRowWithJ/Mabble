...
#define GET_ORIGIN(X) \
	lb.left - X->getPosition().x - (X->get_width() - lb.width) / 2, lb.top - X->getPosition().y - (X->get_height() - lb.height) / 2
...
class CheckButton
{
  ...
  public:
	CheckButton();
	CheckButton(float width, float height, float xPos, float yPos, const char *fontName, float turnXPos, float turnYPos);
